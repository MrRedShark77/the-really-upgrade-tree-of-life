...
export const Composter = {
  get freeFertilizers(): DecimalSource[] {
    const x = [DC.D0,DC.D0,DC.D0]
    if (Weathers.in(1)) return x;
    const B = getBigUpgradeEffect('bacteria\\1')
    for (let i = 0; i < 3; i++) x[i] = x[i].add(B).add(Effect.calculateEffects('fertilizers-'+i, 0));
    return x
  },

  get totalFertilizers(): DecimalSource {
    let x = DC.D0
    const y = this.freeFertilizers
    for (let i = 0; i < 3; i++) x = x.add(player.composter[i].fertilizers).add(y[i]);
    return x
  },
  ...
  get fertilizerBase(): DecimalSource {
    return Decimal.add(1.5, Bacteria.effect(0))
  },
  ...
  calculateFeritizerBulk(i: number, x: DecimalSource): DecimalSource {
    ...
    let y1 = Decimal.max(x, 1).log(this.fertilizers[i].cost[1])
