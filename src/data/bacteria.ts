...
import { scale, softcap } from "@/utils/decimal"
import { Effect, EffectType } from "@/utils/effect"

export const Bacteria = {
  get CellBoost() {
    return scale(Decimal.add(player.bacteria.types, 1), 10, 2, "ME2")
  },
  ...
  get limit() {
    return Effect.calculateEffects('bacteria-limit')
  },

  get speed(): DecimalSource {
    return Decimal.div(getBigUpgradeEffect("bacteria\\2"), 600)
  },

  effects: [
    x => softcap(Decimal.add(x, 1).log(3).mul(.075).root(2).div(10), .5, 1, "LOG"),
