...
export function format(
  ...
  precision: number = 2,
  commas: number = 6,
  notation: string | number = 'mixed_sc',
): string {
  const d = new Decimal(num)
  ...
  switch (notation) {
    ...
    case 'mixed_sc': {
      ...
      if (player.options.notation > 0 && e.lt([3,33,303,3003][player.options.notation])) return format(d, precision, commas, 'st')
...
export const formatMult = (x: DecimalSource, precision?: number, div: boolean = true, div_below: DecimalSource = 1) => div && Decimal.lt(x,div_below) ? "/" + format(Decimal.pow(x, -1)) : "×" + format(x, precision);
...
export function formatGain(a: DecimalSource, e: DecimalSource, f: (x: DecimalSource) => string = format) {
...
export function formatTime(a: DecimalSource, force_hour: boolean = false) {
  ...
  if (Decimal.eq(a, 0)) return "0s";

  const A = new Decimal(a)

  if (A.gte(4.351968e17)) return format(A.div(4.351968e17)) + " uni";
  if (A.gte(3.1536e16)) return format(A.div(3.1536e16)) + " eons";
  ...
  if (A.gte(1)) return format(A) + "s";

  if (A.gte(1e-3)) return format(A.mul(1e3)) + "ms";
  if (A.gte(1e-6)) return format(A.mul(1e6)) + "μs";
  if (A.gte(1e-9)) return format(A.mul(1e9)) + "ns";
  if (A.gte(1e-12)) return format(A.mul(1e12)) + "ps";
  if (A.gte(1e-15)) return format(A.mul(1e15)) + "fs";
  if (A.gte(1e-18)) return format(A.mul(1e18)) + "as";
  if (A.gte(1e-21)) return format(A.mul(1e21)) + "zs";
  if (A.gte(1e-24)) return format(A.mul(1e24)) + "ys";
  if (A.gte(1e-27)) return format(A.mul(1e27)) + "rs";
  if (A.gte(1e-30)) return format(A.mul(1e30)) + "qs";

  return format(A.mul(5.39e44)) + "tP";
}
